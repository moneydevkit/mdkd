...
pub async fn run_event_loop(
	node: Arc<Node>, paginated_store: Arc<dyn PaginatedKVStore>,
	metadata_store: Arc<InvoiceMetadataStore>, webhook_secret: Vec<u8>,
	...
	loop {
		...
		match event {
			Event::PaymentReceived { payment_id, payment_hash, amount_msat, .. } => {
				...
				let payment_id =
					payment_id.expect("PaymentId expected for ldk-server >=0.1");
			...
			Event::ChannelPending { channel_id, counterparty_node_id, .. } => {
				info!("CHANNEL_PENDING: {} from {}", channel_id, counterparty_node_id);
				if let Err(e) = node.event_handled() {
					error!("Failed to mark event as handled: {e}");
				}
			},
			Event::ChannelReady { channel_id, counterparty_node_id, .. } => {
				info!("CHANNEL_READY: {} from {:?}", channel_id, counterparty_node_id);
				if let Err(e) = node.event_handled() {
					error!("Failed to mark event as handled: {e}");
				}
			},
...
fn upsert_payment(
	node: &Node, paginated_store: &Arc<dyn PaginatedKVStore>,
